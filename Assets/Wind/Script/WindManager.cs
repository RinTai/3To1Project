...
public class WindManager : MonoBehaviour
{
    ...
    CommandBuffer cmd;
    public int WindFieldSizeX = 256;
    public int WindFieldSizeY = 32;
    public int WindFieldSizeZ = 256;
    ...
    private static float PopVelocity = 100f;
    ...
    public void OnComputeReadback(AsyncGPUReadbackRequest request)
    {
        if(request.hasError)
        {
            Debug.Log("未读取到RenderTexture");
            return;
        }

        var rawData = request.GetData<Color32>();

        windTexture.SetPixelData(rawData, 0);
        windTexture.Apply();
    }
